...
public class EventController {
    ...
    public String displayEvents(@RequestParam(required = false) Integer categoryId, Model model) {
        if (categoryId == null) {
            ...
            } else { // there are events for that category!
                EventCategory category = result.get();
                model.addAttribute("title", "All Events for Category: " + category.getName());
    ...
    public String displayEventDetails(@RequestParam Integer eventId, Model model) {
        ...
        } else { // there are events for that eventId!
    ...
    // add tag to a specific event : responds to /events/add-tag?eventId=13
    @GetMapping("add-tag")
